...
{
	int resolution = 48; //amount of lines
	float step = (2 * PI) / resolution; // the length of every line

	for (int i = 0; i < resolution; ++i) //this logic repeats 48 times
	{
		float angle = step * i; //0.13 * 0

		float x1 = cos(angle);
		float y1 = sin(angle);

		float next_angle = step * (i + 1); //0.13 * 1
		...
		SDL_RenderDrawLine(
			render,
			x1 * c.radius + c.x, //start x
			y1 * c.radius + c.y, //start y
			x2 * c.radius + c.x, //end x
			y2 * c.radius + c.y  //end y
		);
	}
}

bool circle_intersect(const Circle& a, const Circle& b) // are they intersecting
{
	float dx = b.x - a.x;
	float dy = b.y - a.y;
